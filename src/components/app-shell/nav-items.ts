import {
  Home, HardHat, CalendarDays, CheckSquare, Building2, FileText,
  type LucideIcon,
} from "lucide-react";
...
// 役割でナビの並びを変える。
// スタッフ：日報を中心に（メイン業務）。管理者：マスター管理・全体確認を中心に。
export function navForRole(role: string): NavItem[] {
  if (role === "ADMIN") {
    // 管理者：現場・顧客マスターの管理＋全体確認＋スケジュール
    return [HOME, SITES, CUSTOMERS, CALENDAR, REPORTS, TODOS];
  }
